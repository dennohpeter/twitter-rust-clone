...
pub async fn list(pool: Data<DBPool>) -> HttpResponse {
    ...
    let mut tweets = web::block(move || list_tweets(50, &mut conn))
        .await
        .unwrap();
...
#[post("/tweets")]
pub async fn create(req: Json<TweetRequest>, pool: Data<DBPool>) -> HttpResponse {
    let mut conn = pool.get().expect(CONNECTION_POOL_ERROR);

    let tweet = web::block(move || create_tweet(req.to_tweet().unwrap(), &mut conn))
        .await
        .unwrap();
...
pub async fn get(path: Path<(String,)>, pool: Data<DBPool>) -> HttpResponse {
    ...
    let tweet =
        web::block(move || find_tweet(Uuid::from_str(path.0.as_str()).unwrap(), &mut conn)).await;
...
pub async fn delete(path: Path<(String,)>, pool: Data<DBPool>) -> HttpResponse {
    // in any case return 204
    let mut conn = pool.get().expect(CONNECTION_POOL_ERROR);

    let _ =
        web::block(move || delete_tweet(Uuid::from_str(path.0.as_str()).unwrap(), &mut conn)).await;
